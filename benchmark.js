import loadIris from "./yes/datasets/loadIris.js";
loadIris().then(iris => {
    console.log(iris);
}).catch(error => {
    console.error('Error loading iris dataset:', error);
});
...
//
// pValues.forEach(p => {
//     console.time(`Time with GPU Minkowski distance (p=${p})`);
...
//     console.timeEnd(`Time with GPU Minkowski distance (p=${p})`);
//     console.log(`Accuracy with GPU Minkowski distance (p=${p}): ${accuracy}`);
// });
//
// pValues.forEach(p => {
//     console.time(`Time with CPU Minkowski distance (p=${p})`);
//     const knn = new KNeighborsClassifier(5, 'minkowski', p, 'cpu');
//     knn.fit(X_train, y_train);
//     const y_pred = knn.predict(X_test);
//     const accuracy = accuracyScore(y_test, y_pred);
//     console.timeEnd(`Time with CPU Minkowski distance (p=${p})`);
